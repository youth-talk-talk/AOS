...
class ScrapPolicyRemoteMediator @Inject constructor(
    ...
    override suspend fun load(loadType: LoadType, state: PagingState<Int, Policy>): MediatorResult {
        ...
        try {
            ...
            val response = policyService.getScrapPolicies(
                page = page,
                size = state.config.pageSize
            )
            val policies = response.data?.map { it.toDomain().copy(policyType = policyType) } ?: listOf()
            youthDatabase.withTransaction {
                policyRemoteKeyDao.insertOrReplace(PolicyRemoteKey(nextPage = page + 1, policyType = policyType))
                policyDao.insertAll(policies)
            }
            return MediatorResult.Success(endOfPaginationReached = policies.size != state.config.pageSize)
